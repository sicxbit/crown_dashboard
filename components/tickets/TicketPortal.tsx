...
export default function TicketPortal({ currentUserName, initialTickets }: Props) {
  ...
  const visibleTickets = useMemo(() => {
    return tickets.filter(
      (ticket) => assigneeFilter === "all" || ticket.assignedTo === assigneeFilter
    );
  ...
  const handleSubmit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(apiError ?? "Unable to create ticket");
      }

      const json: unknown = await response.json().catch(() => null);
      if (!isCreateTicketResponse(json) || !json.ticket) {
        throw new Error("Invalid response from server");
      }

      setTickets((current) => [json.ticket, ...current]);
      setTitle("");
      setDescription("");
      setPriority("medium");

      const assigneeName = json.ticket.assignedToName;
      setSuccessMessage(
        json.source === "ai"
  ...
  return (
    <main className="mx-auto flex min-h-screen max-w-4xl flex-col gap-8 bg-transparent p-6 text-slate-900 transition-colors dark:text-slate-100">
      ...
      <section className="rounded-xl border border-slate-200 bg-white p-6 shadow transition-colors dark:border-slate-800 dark:bg-slate-900">
        ...
        {/* ✅ ESLint-safe wrapper to avoid no-misused-promises */}
        <form
          onSubmit={(event) => {
